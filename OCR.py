        ...
        <div className="p-5">
            <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                    <p className="font-bold text-slate-800 dark:text-slate-100 truncate" title={file.name || file.original_name}>{file.name || file.original_name || 'Unknown'}</p>
...
const HomePage = ({
  ...
  const filteredFiles = useMemo(() => files.filter(file => {
      const lowercasedFilter = filterText.toLowerCase(); const searchableContent = [file.id, file.name || file.original_name, file.company_name, file.pn_name].join(' ').toLowerCase();
  ...
  const sortedFiles = useMemo(() => { let sortableItems = [...filteredFiles]; if (sortConfig.key) { sortableItems.sort((a, b) => { const keyMap = { id: ['id'], uploaded_at: ['uploaded_at', 'uploadedAt'], name: ['filename', 'name', 'original_name'], processing_status: ['processing_status'], company_name: ['company_name'], pn_name: ['pn_name'] }; const getValue = (obj, key) => { const pKeys = keyMap[key] || [key]; for (const pKey of pKeys) { if (obj[pKey] !== undefined && obj[pKey] !== null) return obj[pKey]; } return null; }; const valA = getValue(a, sortConfig.key); const valB = getValue(b, sortConfig.key); if (valA === null) return 1; if (valB === null) return -1; if (valA < valB) return sortConfig.direction === 'ascending' ? -1 : 1; if (valA > valB) return sortConfig.direction === 'ascending' ? 1 : -1; return 0; }); } return sortableItems; }, [filteredFiles, sortConfig]);
  ...
  const tableHeaders = [
    // UPDATED: Conditionally render checkbox in header
    { label: isSelectionMode ? <input type="checkbox" className="form-checkbox rounded text-indigo-600 focus:ring-indigo-500" onChange={handleSelectAll} checked={paginatedFiles.length > 0 && selectedIds.size === paginatedFiles.length} /> : '#', key: 'select', sortable: false },
    { label: 'ID', key: 'id', sortable: true }, { label: 'File Name', key: 'name', sortable: false }, { label: 'Company', key: 'company_name', sortable: false }, { label: 'PN Name', key: 'pn_name', sortable: false }, { label: 'Quality Check', key: 'quality_check', sortable: false }, { label: 'Upload Date', key: 'uploaded_at', sortable: true }, { label: 'Processing Status', key: 'processing_status', sortable: true }, { label: 'Similarity Status', key: 'similarity_status', sortable: false }, { label: 'Actions', key: 'actions', sortable: false }
  ];
          ...
          <div className="bg-white dark:bg-slate-800/50 rounded-2xl shadow-sm border border-slate-200/80 dark:border-slate-700/50">
            <div className="p-4 border-b border-slate-200 dark:border-slate-800">
              <div className="flex flex-wrap items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    {/* NEW: Selection Mode Toggle Button */}
            ...
            <div className="p-6">
              {viewMode === 'grid' ? (
                ...
                <div className="overflow-x-auto"><table className="min-w-full text-sm whitespace-nowrap"><thead className="bg-slate-50 dark:bg-slate-900/50"><tr className="border-b-2 border-slate-200 dark:border-slate-800">{tableHeaders.map(h => (<th key={h.key} onClick={h.sortable && h.key !== 'select' ? () => requestSort(h.key) : undefined} className={`px-4 py-4 text-left font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider ${h.sortable && h.key !== 'select' ? 'cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-700' : 'cursor-default'}`}>{h.label} {h.sortable && getSortIcon(h.key)}</th>))}</tr></thead>
                <tbody className="divide-y divide-slate-200 dark:divide-slate-700">{paginatedFiles.length > 0 ? (paginatedFiles.map((file) => { const isSelected = selectedIds.has(file.id); return (<tr key={file.id} onClick={() => isSelectionMode ? handleSelectOne(file.id) : setSelectedFile(file)} className={`transition-colors cursor-pointer ${isSelected ? 'bg-indigo-100 dark:bg-indigo-900/50' : 'hover:bg-indigo-50/50 dark:hover:bg-indigo-900/30 even:bg-slate-50/50 dark:even:bg-slate-800/50'}`}>
                  <td className="px-4 py-2" onClick={e => e.stopPropagation()}>{isSelectionMode ? <input type="checkbox" className="form-checkbox h-4 w-4 rounded text-indigo-600 focus:ring-indigo-500 border-slate-300 dark:border-slate-600 dark:bg-slate-800 dark:checked:bg-indigo-500" checked={isSelected} onChange={() => handleSelectOne(file.id)} /> : <span className="w-4 h-4 block"></span>}</td>
                  <td className="px-4 py-4 text-right text-slate-500 dark:text-slate-400 font-mono">{file.id}</td><td className="px-4 py-4 font-semibold text-slate-800 dark:text-slate-100 truncate">{file.name || file.original_name || 'Unknown'}</td><td className="px-4 py-4 text-slate-500 dark:text-slate-400">{file.company_name || '-'}</td><td className="px-4 py-4 text-slate-500 dark:text-slate-400">{file.pn_name || '-'}</td><td className="px-4 py-4 text-slate-500 dark:text-slate-400"><QualityCheckStatus status={file.quality_check} /></td><td className="px-4 py-4 text-slate-500 dark:text-slate-400">{new Date(file.uploadedAt || file.uploaded_at || Date.now()).toLocaleString('en-US', { month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: true })}</td><td className="px-4 py-4 text-center"><StatusPill status={file.processing_status} /></td><td className="px-4 py-4 text-center"><SimilarityStatusPill status={file.similarity_status} /></td>
                  <td className="px-4 py-4"><div className="flex justify-center items-center gap-2"><button onClick={(e) => { e.stopPropagation(); setSelectedFile(file); }} title="View Details" className="p-2 text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/40 rounded-md transition"><Eye size={18} /></button><button onClick={(e) => { e.stopPropagation(); handleDeleteConfirm(file.id); }} title="Delete" className="p-2 text-slate-500 dark:text-slate-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/40 rounded-md transition"><Trash2 size={18} /></button></div></td>
                  </tr>); })) : (<tr><td colSpan={tableHeaders.length} className="text-center py-16 text-slate-500 dark:text-slate-400"><FolderOpen className="mx-auto text-slate-400 dark:text-slate-500" size={48} /><p className="mt-4 font-semibold text-lg">No Files Found</p><p>Try adjusting your search or filters.</p></td></tr>)}</tbody></table></div>
              )}
            </div>
